...
// Data validation
if ( empty( $first_name ) || empty( $last_name ) || empty( $adhaar ) || empty( $gender ) || empty( $staff_id ) || empty( $email ) || empty( $phone ) || empty( $blood_group ) || empty( $password ) || empty( $address ) ) {
...
$sql = "INSERT INTO `staffs` (`first_name`, `last_name`, `password`, `adhaar`, `gender`, `staff_id`, `email`, `phone`, `blood_group`, `position`, `image_url`, `address`) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$stmt->bind_param( "ssssssssssss", $first_name, $last_name, $password, $adhaar, $gender, $staff_id, $email, $phone, $blood_group, $position, $image_url, $address );
